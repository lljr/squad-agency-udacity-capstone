...
module.exports = {
    ...
    included(parent) {
        ...
        this.setMacrosConfig(_1.MacrosConfig.for(appInstance));
        ...
        if (ownOptions.setOwnConfig) {
            _1.MacrosConfig.for(appInstance).setOwnConfig(source, ownOptions.setOwnConfig);
        }
        if (ownOptions.setConfig) {
            for (let [packageName, config] of Object.entries(ownOptions.setConfig)) {
                _1.MacrosConfig.for(appInstance).setConfig(source, packageName, config);
            }
        }
        if (appInstance.env !== 'production') {
            let macros = _1.MacrosConfig.for(appInstance);
        ...
        // our source code
        this.options.babel.plugins.unshift(_1.MacrosConfig.for(appInstance).babelPluginConfig(this.root));
        ...
        appInstance.toTree = function () {
            _1.MacrosConfig.for(appInstance).finalize();
    ...
    setupPreprocessorRegistry(type, registry) {
        if (type === 'parent') {
            ...
            let { plugins, setConfig } = _1.MacrosConfig.astPlugins(this.parent.root);
